"""
Universe Manager — Definisi semesta saham IDX.
Menyediakan UniverseManager untuk memilih subset saham yang diperdagangkan.
"""
from typing import List, Optional, Dict
import logging

logger = logging.getLogger(__name__)

# ---------------------------------------------------------------------------
# Konstanta: Daftar saham per indeks
# ---------------------------------------------------------------------------

IDX_UNIVERSE = [
    'AALI.JK',
    'ABBA.JK',
    'ABDA.JK',
    'ABMM.JK',
    'ACES.JK',
    'ACST.JK',
    'ADES.JK',
    'ADHI.JK',
    'ADMF.JK',
    'ADMG.JK',
    'ADRO.JK',
    'AGII.JK',
    'AGRO.JK',
    'AGRS.JK',
    'AHAP.JK',
    'AIMS.JK',
    'AISA.JK',
    'AKKU.JK',
    'AKPI.JK',
    'AKRA.JK',
    'AKSI.JK',
    'ALDO.JK',
    'ALKA.JK',
    'ALTO.JK',
    'AMAG.JK',
    'AMFG.JK',
    'AMIN.JK',
    'AMRT.JK',
    'ANJT.JK',
    'ANTM.JK',
    'APEX.JK',
    'APIC.JK',
    'APII.JK',
    'APLI.JK',
    'APLN.JK',
    'ARGO.JK',
    'ARII.JK',
    'ARNA.JK',
    'ARTA.JK',
    'ARTO.JK',
    'ASBI.JK',
    'ASDM.JK',
    'ASGR.JK',
    'ASII.JK',
    'ASJT.JK',
    'ASMI.JK',
    'ASRI.JK',
    'ASRM.JK',
    'ASSA.JK',
    'ATIC.JK',
    'AUTO.JK',
    'BABP.JK',
    'BACA.JK',
    'BAJA.JK',
    'BALI.JK',
    'BAPA.JK',
    'BATA.JK',
    'BAYU.JK',
    'BBCA.JK',
    'BBHI.JK',
    'BBKP.JK',
    'BBLD.JK',
    'BBMD.JK',
    'BBNI.JK',
    'BBRI.JK',
    'BBRM.JK',
    'BBTN.JK',
    'BBYB.JK',
    'BCAP.JK',
    'BCIC.JK',
    'BCIP.JK',
    'BDMN.JK',
    'BEKS.JK',
    'BEST.JK',
    'BFIN.JK',
    'BGTG.JK',
    'BHIT.JK',
    'BIMA.JK',
    'BINA.JK',
    'BIPI.JK',
    'BIPP.JK',
    'BIRD.JK',
    'BISI.JK',
    'BJBR.JK',
    'BJTM.JK',
    'BKDP.JK',
    'BKSL.JK',
    'BKSW.JK',
    'BLTA.JK',
    'BLTZ.JK',
    'BMAS.JK',
    'BMRI.JK',
    'BMSR.JK',
    'BMTR.JK',
    'BNBA.JK',
    'BNBR.JK',
    'BNGA.JK',
    'BNII.JK',
    'BNLI.JK',
    'BOLT.JK',
    'BPFI.JK',
    'BPII.JK',
    'BRAM.JK',
    'BRMS.JK',
    'BRNA.JK',
    'BRPT.JK',
    'BSDE.JK',
    'BSIM.JK',
    'BSSR.JK',
    'BSWD.JK',
    'BTEK.JK',
    'BTON.JK',
    'BTPN.JK',
    'BUDI.JK',
    'BUKK.JK',
    'BULL.JK',
    'BUMI.JK',
    'BUVA.JK',
    'BVIC.JK',
    'BWPT.JK',
    'BYAN.JK',
    'CANI.JK',
    'CASS.JK',
    'CEKA.JK',
    'CENT.JK',
    'CFIN.JK',
    'CINT.JK',
    'CITA.JK',
    'CLPI.JK',
    'CMNP.JK',
    'CMPP.JK',
    'CNKO.JK',
    'CNTX.JK',
    'CPIN.JK',
    'CPRO.JK',
    'CSAP.JK',
    'CTBN.JK',
    'CTRA.JK',
    'CTTH.JK',
    'DART.JK',
    'DEFI.JK',
    'DEWA.JK',
    'DGIK.JK',
    'DILD.JK',
    'DKFT.JK',
    'DLTA.JK',
    'DMAS.JK',
    'DNAR.JK',
    'DNET.JK',
    'DOID.JK',
    'DPNS.JK',
    'DSFI.JK',
    'DSNG.JK',
    'DSSA.JK',
    'DUTI.JK',
    'DVLA.JK',
    'DYAN.JK',
    'ECII.JK',
    'EKAD.JK',
    'ELSA.JK',
    'ELTY.JK',
    'EMDE.JK',
    'EMTK.JK',
    'ENRG.JK',
    'EPMT.JK',
    'ERAA.JK',
    'ERTX.JK',
    'ESSA.JK',
    'ESTI.JK',
    'EXCL.JK',
    'FAST.JK',
    'FISH.JK',
    'FMII.JK',
    'FORU.JK',
    'FPNI.JK',
    'GDST.JK',
    'GDYR.JK',
    'GEMA.JK',
    'GEMS.JK',
    'GGRM.JK',
    'GIAA.JK',
    'GJTL.JK',
    'GLOB.JK',
    'GMTD.JK',
    'GOLD.JK',
    'GPRA.JK',
    'GSMF.JK',
    'GTBO.JK',
    'GWSA.JK',
    'GZCO.JK',
    'HADE.JK',
    'HDFA.JK',
    'HERO.JK',
    'HEXA.JK',
    'HITS.JK',
    'HMSP.JK',
    'HRUM.JK',
    'IATA.JK',
    'IBFN.JK',
    'IBST.JK',
    'ICBP.JK',
    'ICON.JK',
    'IGAR.JK',
    'IKAI.JK',
    'IKBI.JK',
    'IMAS.JK',
    'IMJS.JK',
    'IMPC.JK',
    'INAI.JK',
    'INCI.JK',
    'INCO.JK',
    'INDF.JK',
    'INDR.JK',
    'INDS.JK',
    'INDX.JK',
    'INDY.JK',
    'INKP.JK',
    'INPC.JK',
    'INPP.JK',
    'INRU.JK',
    'INTA.JK',
    'INTD.JK',
    'INTP.JK',
    'IPOL.JK',
    'ISAT.JK',
    'ISSP.JK',
    'ITMA.JK',
    'ITMG.JK',
    'JAWA.JK',
    'JECC.JK',
    'JIHD.JK',
    'JKON.JK',
    'JPFA.JK',
    'JRPT.JK',
    'JSMR.JK',
    'JSPT.JK',
    'JTPE.JK',
    'KAEF.JK',
    'KARW.JK',
    'KBLI.JK',
    'KBLM.JK',
    'KBLV.JK',
    'KDSI.JK',
    'KIAS.JK',
    'KICI.JK',
    'KIJA.JK',
    'KKGI.JK',
    'KLBF.JK',
    'KOBX.JK',
    'KOIN.JK',
    'KONI.JK',
    'KOPI.JK',
    'KPIG.JK',
    'KRAS.JK',
    'KREN.JK',
    'LAPD.JK',
    'LEAD.JK',
    'LINK.JK',
    'LION.JK',
    'LMPI.JK',
    'LPCK.JK',
    'LPGI.JK',
    'LPIN.JK',
    'LPKR.JK',
    'LPLI.JK',
    'LPPF.JK',
    'LPPS.JK',
    'LRNA.JK',
    'LSIP.JK',
    'LTLS.JK',
    'MAIN.JK',
    'MAPI.JK',
    'MAYA.JK',
    'MBAP.JK',
    'MBSS.JK',
    'MBTO.JK',
    'MDIA.JK',
    'MDKA.JK',
    'MDLN.JK',
    'MDRN.JK',
    'MEDC.JK',
    'MEGA.JK',
    'MERK.JK',
    'META.JK',
    'MGNA.JK',
    'MICE.JK',
    'MIDI.JK',
    'MIKA.JK',
    'MIRA.JK',
    'MITI.JK',
    'MKPI.JK',
    'MLBI.JK',
    'MLIA.JK',
    'MLPL.JK',
    'MLPT.JK',
    'MMLP.JK',
    'MNCN.JK',
    'MPMX.JK',
    'MPPA.JK',
    'MRAT.JK',
    'MREI.JK',
    'MSKY.JK',
    'MTDL.JK',
    'MTFN.JK',
    'MTLA.JK',
    'MYOH.JK',
    'MYOR.JK',
    'NELY.JK',
    'NIKL.JK',
    'NIRO.JK',
    'NISP.JK',
    'NOBU.JK',
    'NRCA.JK',
    'OCAP.JK',
    'OKAS.JK',
    'OMRE.JK',
    'PADI.JK',
    'PALM.JK',
    'PANR.JK',
    'PANS.JK',
    'PBRX.JK',
    'PDES.JK',
    'PEGE.JK',
    'PGAS.JK',
    'PGLI.JK',
    'PICO.JK',
    'PJAA.JK',
    'PKPK.JK',
    'PNBN.JK',
    'PNBS.JK',
    'PNIN.JK',
    'PNLF.JK',
    'PNSE.JK',
    'POLY.JK',
    'PSAB.JK',
    'PSDN.JK',
    'PSKT.JK',
    'PTBA.JK',
    'PTIS.JK',
    'PTPP.JK',
    'PTRO.JK',
    'PTSN.JK',
    'PTSP.JK',
    'PUDP.JK',
    'PWON.JK',
    'PYFA.JK',
    'RAJA.JK',
    'RALS.JK',
    'RANC.JK',
    'RBMS.JK',
    'RDTX.JK',
    'RELI.JK',
    'RICY.JK',
    'RIGS.JK',
    'RODA.JK',
    'ROTI.JK',
    'RUIS.JK',
    'SAFE.JK',
    'SAME.JK',
    'SCCO.JK',
    'SCMA.JK',
    'SCPI.JK',
    'SDMU.JK',
    'SDPC.JK',
    'SDRA.JK',
    'SGRO.JK',
    'SHID.JK',
    'SIDO.JK',
    'SILO.JK',
    'SIMP.JK',
    'SIPD.JK',
    'SKBM.JK',
    'SKLT.JK',
    'SMAR.JK',
    'SMBR.JK',
    'SMDM.JK',
    'SMDR.JK',
    'SMGR.JK',
    'SMMA.JK',
    'SMMT.JK',
    'SMRA.JK',
    'SOCI.JK',
    'SPMA.JK',
    'SQMI.JK',
    'SRSN.JK',
    'SRTG.JK',
    'SSIA.JK',
    'SSMS.JK',
    'SSTM.JK',
    'STAR.JK',
    'STTP.JK',
    'SULI.JK',
    'TARA.JK',
    'TAXI.JK',
    'TBLA.JK',
    'TBMS.JK',
    'TCID.JK',
    'TFCO.JK',
    'TGKA.JK',
    'TIFA.JK',
    'TINS.JK',
    'TIRA.JK',
    'TIRT.JK',
    'TKIM.JK',
    'TLKM.JK',
    'TMAS.JK',
    'TMPO.JK',
    'TOBA.JK',
    'TOTL.JK',
    'TOTO.JK',
    'TOWR.JK',
    'TPIA.JK',
    'TPMA.JK',
    'TRIM.JK',
    'TRIS.JK',
    'TRST.JK',
    'TRUS.JK',
    'TSPC.JK',
    'ULTJ.JK',
    'UNIC.JK',
    'UNSP.JK',
    'UNTR.JK',
    'UNVR.JK',
    'VICO.JK',
    'VINS.JK',
    'VIVA.JK',
    'VOKS.JK',
    'VRNA.JK',
    'WAPO.JK',
    'WEHA.JK',
    'WIIM.JK',
    'WINS.JK',
    'WOMF.JK',
    'WTON.JK',
    'YPAS.JK',
    'YULE.JK',
    'ZBRA.JK',
    'SHIP.JK',
    'CASA.JK',
    'DPUM.JK',
    'IDPR.JK',
    'JGLE.JK',
    'KINO.JK',
    'MARI.JK',
    'OASA.JK',
    'POWR.JK',
    'INCF.JK',
    'WSBP.JK',
    'PBSA.JK',
    'PRDA.JK',
    'BOGA.JK',
    'BRIS.JK',
    'PORT.JK',
    'CARS.JK',
    'MINA.JK',
    'CLEO.JK',
    'TAMU.JK',
    'CSIS.JK',
    'FIRE.JK',
    'KMTR.JK',
    'MAPB.JK',
    'WOOD.JK',
    'HRTA.JK',
    'HOKI.JK',
    'MPOW.JK',
    'MARK.JK',
    'NASA.JK',
    'MDKI.JK',
    'BELL.JK',
    'KIOS.JK',
    'GMFI.JK',
    'MTWI.JK',
    'ZINC.JK',
    'MCAS.JK',
    'PPRE.JK',
    'WEGE.JK',
    'PSSI.JK',
    'MORA.JK',
    'DWGL.JK',
    'PBID.JK',
    'JMAS.JK',
    'CAMP.JK',
    'IPCM.JK',
    'PCAR.JK',
    'LCKM.JK',
    'HELI.JK',
    'GHON.JK',
    'NICK.JK',
    'BTPS.JK',
    'SPTO.JK',
    'TRUK.JK',
    'TUGU.JK',
    'MSIN.JK',
    'MAPA.JK',
    'TCPI.JK',
    'IPCC.JK',
    'RISE.JK',
    'BPTR.JK',
    'NFCX.JK',
    'MGRO.JK',
    'FILM.JK',
    'ANDI.JK',
    'LAND.JK',
    'MOLI.JK',
    'PANI.JK',
    'DIGI.JK',
    'CITY.JK',
    'SAPX.JK',
    'SURE.JK',
    'MPRO.JK',
    'GOOD.JK',
    'SKRN.JK',
    'YELO.JK',
    'CAKK.JK',
    'SATU.JK',
    'SOSS.JK',
    'POLA.JK',
    'DIVA.JK',
    'LUCK.JK',
    'URBN.JK',
    'SOTS.JK',
    'ZONE.JK',
    'PEHA.JK',
    'FOOD.JK',
    'BEEF.JK',
    'POLI.JK',
    'CLAY.JK',
    'NATO.JK',
    'JAYA.JK',
    'COCO.JK',
    'MTPS.JK',
    'HRME.JK',
    'JAST.JK',
    'FITT.JK',
    'BOLA.JK',
    'CCSI.JK',
    'SFAN.JK',
    'POLU.JK',
    'KJEN.JK',
    'ITIC.JK',
    'PAMG.JK',
    'IPTV.JK',
    'BLUE.JK',
    'EAST.JK',
    'LIFE.JK',
    'FUJI.JK',
    'KOTA.JK',
    'INOV.JK',
    'ARKA.JK',
    'SMKL.JK',
    'HDIT.JK',
    'KEEN.JK',
    'BAPI.JK',
    'GGRP.JK',
    'OPMS.JK',
    'NZIA.JK',
    'SLIS.JK',
    'IRRA.JK',
    'DMMX.JK',
    'SINI.JK',
    'ESIP.JK',
    'TEBE.JK',
    'KEJU.JK',
    'PSGO.JK',
    'IFSH.JK',
    'REAL.JK',
    'IFII.JK',
    'PMJS.JK',
    'UCID.JK',
    'GLVA.JK',
    'PGJO.JK',
    'AMAR.JK',
    'CSRA.JK',
    'INDO.JK',
    'AMOR.JK',
    'TRIN.JK',
    'DMND.JK',
    'PURA.JK',
    'PTPW.JK',
    'TAMA.JK',
    'IKAN.JK',
    'SAMF.JK',
    'KBAG.JK',
    'RONY.JK',
    'CSMI.JK',
    'BBSS.JK',
    'BHAT.JK',
    'CASH.JK',
    'EPAC.JK',
    'UANG.JK',
    'PGUN.JK',
    'SOFA.JK',
    'PPGL.JK',
    'SGER.JK',
    'TRJA.JK',
    'PNGO.JK',
    'SCNP.JK',
    'BBSI.JK',
    'KMDS.JK',
    'PURI.JK',
    'SOHO.JK',
    'HOMI.JK',
    'ROCK.JK',
    'ENZO.JK',
    'PLAN.JK',
    'PTDU.JK',
    'ATAP.JK',
    'VICI.JK',
    'PMMP.JK',
    'BANK.JK',
    'WMUU.JK',
    'EDGE.JK',
    'UNIQ.JK',
    'SNLK.JK',
    'ZYRX.JK',
    'LFLO.JK',
    'FIMP.JK',
    'TAPG.JK',
    'LUCY.JK',
    'ADCP.JK',
    'HOPE.JK',
    'MGLV.JK',
    'TRUE.JK',
    'ARCI.JK',
    'IPAC.JK',
    'MASB.JK',
    'BMHS.JK',
    'FLMC.JK',
    'NICL.JK',
    'UVCR.JK',
    'BUKA.JK',
    'HAIS.JK',
    'OILS.JK',
    'GPSO.JK',
    'MCOL.JK',
    'RUNS.JK',
    'SBMA.JK',
    'CMNT.JK',
    'GTSI.JK',
    'IDEA.JK',
    'KUAS.JK',
    'BOBA.JK',
    'MTEL.JK',
    'DEPO.JK',
    'BINO.JK',
    'CMRY.JK',
    'WGSH.JK',
    'TAYS.JK',
    'RMKE.JK',
    'OBMD.JK',
    'AVIA.JK',
    'NASI.JK',
    'BSML.JK',
    'DRMA.JK',
    'ADMR.JK',
    'SEMA.JK',
    'ASLC.JK',
    'NETV.JK',
    'BAUT.JK',
    'ENAK.JK',
    'NTBK.JK',
    'SMKM.JK',
    'STAA.JK',
    'NANO.JK',
    'BIKE.JK',
    'WIRG.JK',
    'SICO.JK',
    'GOTO.JK',
    'TLDN.JK',
    'MTMH.JK',
    'WINR.JK',
    'IBOS.JK',
    'OLIV.JK',
    'ASHA.JK',
    'SWID.JK',
    'TRGU.JK',
    'ARKO.JK',
    'CHEM.JK',
    'DEWI.JK',
    'AXIO.JK',
    'KRYA.JK',
    'HATM.JK',
    'RCCC.JK',
    'GULA.JK',
    'JARR.JK',
    'AMMS.JK',
    'RAFI.JK',
    'KKES.JK',
    'ELPI.JK',
    'EURO.JK',
    'KLIN.JK',
    'TOOL.JK',
    'BUAH.JK',
    'CRAB.JK',
    'MEDS.JK',
    'COAL.JK',
    'PRAY.JK',
    'CBUT.JK',
    'BELI.JK',
    'MKTR.JK',
    'OMED.JK',
    'BSBK.JK',
    'PDPP.JK',
    'KDTN.JK',
    'ZATA.JK',
    'NINE.JK',
    'MMIX.JK',
    'PADA.JK',
    'ISAP.JK',
    'VTNY.JK',
    'SOUL.JK',
    'ELIT.JK',
    'BEER.JK',
    'CBPE.JK',
    'SUNI.JK',
    'CBRE.JK',
    'WINE.JK',
    'BMBL.JK',
    'PEVE.JK',
    'LAJU.JK',
    'FWCT.JK',
    'NAYZ.JK',
    'IRSX.JK',
    'PACK.JK',
    'VAST.JK',
    'CHIP.JK',
    'HALO.JK',
    'KING.JK',
    'PGEO.JK',
    'FUTR.JK',
    'HILL.JK',
    'BDKR.JK',
    'PTMP.JK',
    'SAGE.JK',
    'TRON.JK',
    'CUAN.JK',
    'NSSS.JK',
    'GTRA.JK',
    'HAJJ.JK',
    'JATI.JK',
    'TYRE.JK',
    'MPXL.JK',
    'SMIL.JK',
    'KLAS.JK',
    'MAXI.JK',
    'VKTR.JK',
    'RELF.JK',
    'AMMN.JK',
    'CRSN.JK',
    'GRPM.JK',
    'WIDI.JK',
    'INET.JK',
    'MAHA.JK',
    'RMKO.JK',
    'CNMA.JK',
    'FOLK.JK',
    'HBAT.JK',
    'GRIA.JK',
    'PPRI.JK',
    'ERAL.JK',
    'CYBR.JK',
    'MUTU.JK',
    'LMAX.JK',
    'HUMI.JK',
    'MSIE.JK',
    'RSCH.JK',
    'BABY.JK',
    'AEGS.JK',
    'IOTF.JK',
    'KOCI.JK',
    'PTPS.JK',
    'BREN.JK',
    'STRK.JK',
    'KOKA.JK',
    'LOPI.JK',
    'UDNG.JK',
    'RGAS.JK',
    'MSTI.JK',
    'IKPM.JK',
    'AYAM.JK',
    'SURI.JK',
    'ASLI.JK',
    'GRPH.JK',
    'SMGA.JK',
    'UNTD.JK',
    'TOSK.JK',
    'MPIX.JK',
    'ALII.JK',
    'MKAP.JK',
    'MEJA.JK',
    'LIVE.JK',
    'HYGN.JK',
    'BAIK.JK',
    'VISI.JK',
    'AREA.JK',
    'MHKI.JK',
    'ATLA.JK',
    'DATA.JK',
    'SOLA.JK',
    'BATR.JK',
    'SPRE.JK',
    'PART.JK',
    'GOLF.JK',
    'ISEA.JK',
    'BLES.JK',
    'GUNA.JK',
    'LABS.JK',
    'DOSS.JK',
    'NEST.JK',
    'VERN.JK',
    'DAAZ.JK',
    'BOAT.JK',
    'NAIK.JK',
    'AADI.JK',
    'MDIY.JK',
    'KSIX.JK',
    'RATU.JK',
    'YOII.JK',
    'HGII.JK',
    'BRRC.JK',
    'DGWG.JK',
    'CBDK.JK',
    'OBAT.JK',
    'MINE.JK',
    'ASPR.JK',
    'PSAT.JK',
    'COIN.JK',
    'CDIA.JK',
    'BLOG.JK',
    'MERI.JK',
    'CHEK.JK',
    'PMUI.JK',
    'EMAS.JK',
    'PJHB.JK',
    'RLCO.JK',
    'SUPA.JK',
    'KAQI.JK',
    'YUPI.JK',
    'FORE.JK',
    'MDLA.JK',
    'DKHH.JK',
    'AYLS.JK',
    'DADA.JK',
    'ASPI.JK',
    'ESTA.JK',
    'BESS.JK',
    'AMAN.JK',
    'CARE.JK',
    'PIPA.JK',
    'NCKL.JK',
    'MENN.JK',
    'AWAN.JK',
    'MBMA.JK',
    'RAAM.JK',
    'DOOH.JK',
    'CGAS.JK',
    'NICE.JK',
    'SMLE.JK',
    'MANG.JK',
    'WIFI.JK',
    'FAPA.JK',
    'DCII.JK',
    'KETR.JK',
    'DGNS.JK',
    'UFOE.JK',
    'DFAM.JK',
    'PRIM.JK',
    'SWAT.JK',
    'NPGF.JK',
    'LABA.JK',
    'INPS.JK',
    'HEAL.JK',
    'PZZA.JK',
    'TNCA.JK',
    'TFAS.JK',
    'WOWS.JK',
    'AGAR.JK',
    'MCOR.JK',
    'SMSM.JK',
    'SONA.JK',
    'SRAJ.JK',
    'TALF.JK',
    'TBIG.JK',
    'DAYA.JK',
    'MSJA.JK',
    'ACRO.JK'
]

# ---------------------------------------------------------------------------
# Subset indeks populer
# ---------------------------------------------------------------------------

LQ45: List[str] = [
    "BBCA.JK", "BBRI.JK", "BMRI.JK", "TLKM.JK", "ASII.JK",
    "BBNI.JK", "UNVR.JK", "ICBP.JK", "INDF.JK", "KLBF.JK",
    "ADRO.JK", "PTBA.JK", "ITMG.JK", "UNTR.JK", "SMGR.JK",
    "GGRM.JK", "HMSP.JK", "EXCL.JK", "ISAT.JK", "BRIS.JK",
    "MDKA.JK", "ANTM.JK", "INCO.JK", "TINS.JK", "AKRA.JK",
    "PGAS.JK", "MEDC.JK", "BSDE.JK", "PWON.JK", "CTRA.JK",
    "JPFA.JK", "CPIN.JK", "ERAA.JK", "ACES.JK", "MAPI.JK",
    "JSMR.JK", "TOWR.JK", "TBIG.JK", "SCMA.JK", "EMTK.JK",
    "MYOR.JK", "SIDO.JK", "AMRT.JK", "MNCN.JK", "SRIL.JK",
]

KOMPAS100: List[str] = [
    # LQ45 core
    "BBCA.JK", "BBRI.JK", "BMRI.JK", "TLKM.JK", "ASII.JK",
    "BBNI.JK", "UNVR.JK", "ICBP.JK", "INDF.JK", "KLBF.JK",
    "ADRO.JK", "PTBA.JK", "ITMG.JK", "UNTR.JK", "SMGR.JK",
    "GGRM.JK", "HMSP.JK", "EXCL.JK", "ISAT.JK", "BRIS.JK",
    "MDKA.JK", "ANTM.JK", "INCO.JK", "TINS.JK", "AKRA.JK",
    "PGAS.JK", "MEDC.JK", "BSDE.JK", "PWON.JK", "CTRA.JK",
    "JPFA.JK", "CPIN.JK", "ERAA.JK", "ACES.JK", "MAPI.JK",
    "JSMR.JK", "TOWR.JK", "TBIG.JK", "SCMA.JK", "EMTK.JK",
    "MYOR.JK", "SIDO.JK", "AMRT.JK", "MNCN.JK", "SRIL.JK",
    # Extended Kompas100
    "BBTN.JK", "BDMN.JK", "BTPN.JK", "MEGA.JK", "BNGA.JK",
    "BNLI.JK", "NISP.JK", "BJBR.JK", "BJTM.JK", "SDRA.JK",
    "BRPT.JK", "TPIA.JK", "INKP.JK", "TKIM.JK", "SMRA.JK",
    "LPKR.JK", "DILD.JK", "INTP.JK", "WIKA.JK", "PTPP.JK",
    "AUTO.JK", "SMSM.JK", "GJTL.JK", "LPPF.JK", "RALS.JK",
    "MLBI.JK", "DLTA.JK", "TSPC.JK", "MERK.JK", "DVLA.JK",
    "HRUM.JK", "DOID.JK", "BSSR.JK", "MYOH.JK", "MBAP.JK",
    "ELSA.JK", "ESSA.JK", "INDY.JK", "DSSA.JK", "TOBA.JK",
    "LINK.JK", "MDIA.JK", "MTEL.JK", "SILO.JK", "MIKA.JK",
    "GEMS.JK", "MDIY.JK", "PGEO.JK", "BREN.JK", "CUAN.JK",
    "AMMN.JK", "NCKL.JK", "TAPG.JK", "GOTO.JK", "BUKA.JK",
]

UNIVERSE_OPTIONS: Dict[str, List[str]] = {
    "idx_all":   IDX_UNIVERSE,
    "lq45":      LQ45,
    "kompas100": KOMPAS100,
    "custom":    [],
}


class UniverseManager:
    """
    Mengelola semesta saham yang akan diperdagangkan.

    Args:
        universe_name: Nama universe ("idx_all", "lq45", "kompas100", "custom")
        custom_tickers: List ticker custom (wajib jika universe_name="custom")
    """

    def __init__(
        self,
        universe_name: str = "lq45",
        custom_tickers: Optional[List[str]] = None,
    ):
        if universe_name == "custom":
            if not custom_tickers:
                raise ValueError("custom_tickers wajib diisi jika universe_name='custom'")
            self.tickers: List[str] = sorted(custom_tickers)
        elif universe_name in UNIVERSE_OPTIONS:
            self.tickers = sorted(UNIVERSE_OPTIONS[universe_name])
        else:
            raise ValueError(
                f"Unknown universe '{universe_name}'. "
                f"Available: {list(UNIVERSE_OPTIONS.keys())}"
            )

        self.universe_name = universe_name
        logger.info(
            "UniverseManager initialized: %s (%d tickers)",
            self.universe_name, len(self.tickers),
        )

    def get_tickers(self) -> List[str]:
        """Return list ticker yang aktif."""
        return self.tickers

    def __repr__(self) -> str:
        return f"UniverseManager(name='{self.universe_name}', n_tickers={len(self.tickers)})"